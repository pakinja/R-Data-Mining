...
agg <- function(tienda){
  aggregate(tienda[,c("TOTAL")], by=list(tienda$TIPO_PAGO), "sum")
}
...
pl<- function(tienda,k){
  ...
  plot_tallas <- ggplot(tienda, aes(x = reorder(Group.1,-x), y = x)) + 
    ...
    scale_y_continuous(limits=c(0, 6.5), breaks = seq(0,6.5,by = 0.5))+
    labs(x="PAGO", y="% respecto al total de venta")+
