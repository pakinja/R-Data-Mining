...
agg <- function(tienda){
  aggregate(tienda[,c("TOTAL")], by=list(tienda$CODIGO_SKU), "sum")
}
...
pl<- function(tienda,k){
  ...
  plot_tallas <- ggplot(tienda, aes(x = reorder(Group.1,-x), y = x)) + 
    geom_bar(stat = "identity", fill="red", colour="blue")+
    #ylim(c(0, 0.3, 0.01))+
    theme(axis.text.x = element_text(angle = 45, hjust = 1))+
    scale_y_continuous(limits=c(0, 0.009), breaks = seq(0,0.009,by = 0.001))+
    labs(x="COLOR", y="% respecto al total de venta")+
