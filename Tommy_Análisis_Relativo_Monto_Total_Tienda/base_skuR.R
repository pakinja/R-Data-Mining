...
dcto <- as.factor(gsub(" ","", pv$DESCUENTO))
...
pvdcto <- aggregate(pv[,c("CANTIDAD")], by=list(pv$DESCUENTO), "sum")
...
pvdcto <- pvdcto[order(pvdcto[[2]], decreasing = TRUE),]
...
descuentos <- pvdcto[1:muestra,1]

# filtramos dataframe por top categorias
fil.sku <- pv[pv$CODIGO_SKU %in% skus,]
...
DESCUENTO <- factor(DESCUENTO, levels=descuentos)
#
...
freqDESC <- table(DESCUENTO)
relfreqDESC <- table(DESCUENTO)/length(DESCUENTO)
cbind(freqDESC, relfreqDESC)
...
barplot(joint_counts_sku_id, beside = TRUE, col=1:10, ylab = "Frequency", xlab = "ID_TIENDA")
title("TOMMY")
legend('topright', levels(CODIGO_SKU), pch=15, col=1:10, ncol = 4,  cex = 0.75)

joint_sku_desc <- CrossTable(CODIGO_SKU, DESCUENTO, prop.chisq = FALSE)
joint_counts_sku_desc <- joint_sku_desc$t

barplot(joint_counts_sku_desc, beside = TRUE, col=rainbow(10, s=0.5), ylab = "Frequency", xlab = "DESCUENTO")
title("TOMMY")
legend('topleft', levels(CODIGO_SKU), pch=15, col=rainbow(10, s=0.5), ncol = 2,  cex = 0.75)
