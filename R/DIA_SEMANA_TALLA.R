...
fil.cat <- pv[pv$FECHA_CREACION %in% pvA[1:7, 1] & pv$TALLA %in% pvB[1:10, 1],]
...
barplot(joint_counts_diames_talla, beside = TRUE, col=brewer.pal(10,"Paired"),
        ylab = "No. Artículos", xlab = "Día de la Semana y TALLA")
title(main)
legend('top', levels(TALLA), pch=15, col=brewer.pal(10,"Paired"), ncol = 4,  cex = 1)
