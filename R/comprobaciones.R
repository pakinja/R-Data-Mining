...
kkk <- filter(pv, ID_TIENDA==unique(unique(pv$ID_TIENDA)[10]))
kkk <- aggregate(kkk[,c("TOTAL")], by=list(kkk$NO_TRANSACCION), "sum")
kkk <- kkk[order(kkk$x, decreasing = TRUE),]


lll <- filter(pv, NO_TRANSACCION == 155651 & ID_TIENDA == 3010)
######################################################################

ooo <- aggregate(pv[,c("TOTAL")], by=list(pv$ID_TIENDA,pv$NO_TRANSACCION), "sum")


ppp <- filter(pv, ID_TIENDA==unique(unique(pv$ID_TIENDA)[1]))
ppp <- aggregate(ppp[,c("TOTAL")], by=list(ppp$NO_TRANSACCION), "sum")
