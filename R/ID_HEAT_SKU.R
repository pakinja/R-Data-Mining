...
pv$CODIGO_SKU<- as.factor(pv$CODIGO_SKU)
pv$NO_TRANSACCION <- factor(pv$NO_TRANSACCION )
pv$CODIGO_SKU <- factor(pv$CODIGO_SKU)
...
pl<- function(tienda,k){
  ...
  pp <- ggplot(tienda, aes(x=NO_TRANSACCION, y=CODIGO_SKU))+
    geom_bin2d()+
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          panel.grid.major = element_line(colour = "orange", size =.6))+
    #labs(x="NO_TRANSACCION", y="")+
    ggtitle(paste("COACH", as.character(k)))
