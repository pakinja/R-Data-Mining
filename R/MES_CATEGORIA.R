...
fil.cat <- pv[pv$FECHA_CREACION %in% pvA[1:12, 1] & pv$CATEGORIA %in% pvB[1:10, 1],]
...
joint_mes_categoria <- CrossTable(CATEGORIA, FECHA_CREACION, prop.chisq = FALSE)
joint_counts_mes_categoria <- joint_mes_categoria$t

barplot(joint_counts_mes_categoria, beside = TRUE, col=brewer.pal(10,"Paired"),
        ylab = "No. Artículos", xlab = "Mes & CATEGORIA")
title(main)
legend('top', levels(CATEGORIA), pch=15, col=brewer.pal(10,"Paired"), ncol = 4,  cex = 1)
