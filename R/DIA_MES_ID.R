...
fil.cat <- pv[pv$FECHA_CREACION %in% pvA[1:31, 1] & pv$ID_TIENDA %in% pvB[1:9, 1],]
...
barplot(joint_counts_diames_id, beside = TRUE, col=brewer.pal(9,"Paired"),
        ylab = "No. Artículos", xlab = "Día del Mes & ID_TIENDA")
title(main)
legend('topright', levels(ID_TIENDA), pch=15, col=brewer.pal(9,"Paired"), ncol = 4,  cex = 1)
