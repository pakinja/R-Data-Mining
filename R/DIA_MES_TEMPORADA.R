...
fil.cat <- pv[pv$FECHA_CREACION %in% pvA[1:31, 1] & pv$TEMPORADA %in% pvB[1:9, 1],]
...
barplot(joint_counts_diames_temporada, beside = TRUE, col=brewer.pal(9,"Paired"),
        ylab = "No. Artículos", xlab = "Día del Mes & TEMPORADA")
title(main)
legend('topleft', levels(TEMPORADA), pch=15, col=brewer.pal(9,"Paired"), ncol = 4,  cex = 1)
