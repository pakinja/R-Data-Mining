...
pvpgo <- aggregate(pv[,c("CANTIDAD")], by=list(pv$TIPO_PAGO, pv$NO_TRANSACCION), "sum")
...
barplot(joint_counts_id_talla, beside = TRUE, col=brewer.pal(9,"Paired"),
        ylab = "% Respecto a la cantidad total de artículos vendidos", xlab = "ID_TIENDA")
title("TOMMY: 1079262 artículos @176175 transacciones @427 días 2015-11-01 - 2016-12-31 total = $800,726,791")
legend('topright', levels(TALLA), pch=15, col=brewer.pal(9,"Paired"), ncol = 4,  cex = 1)

joint_id_cat <- CrossTable(CATEGORIA,ID_TIENDA, prop.chisq = FALSE)
joint_counts_id_cat <- (joint_id_cat$t/sum(pv$CANTIDAD)*100)

barplot(joint_counts_id_cat, beside = TRUE, col=brewer.pal(10,"Paired"),
        ylab = "% Respecto a la cantidad total de artículos vendidos", xlab = "ID_TIENDA")
title("TOMMY: 1079262 artículos @176175 transacciones @427 días 2015-11-01 - 2016-12-31 total = $800,726,791")
legend('topright', levels(CATEGORIA), pch=15, col=brewer.pal(10,"Paired"), ncol = 4,  cex = 1)

joint_id_color <- CrossTable(COLOR,ID_TIENDA, prop.chisq = FALSE)
joint_counts_id_color <- (joint_id_color$t/sum(pv$CANTIDAD)*100)

barplot(joint_counts_id_color, beside = TRUE, col=brewer.pal(10,"Paired"),
        ylab = "% Respecto a la cantidad total de artículos vendidos", xlab = "ID_TIENDA")
title("TOMMY: 1079262 artículos @176175 transacciones @427 días 2015-11-01 - 2016-12-31 total = $800,726,791")
legend('topright', levels(COLOR), pch=15, col=brewer.pal(10,"Paired"), ncol = 4,  cex = 1)

joint_id_tpago <- CrossTable(TIPO_PAGO,ID_TIENDA, prop.chisq = FALSE)
joint_counts_id_tpago <- (joint_id_tpago$t/sum(pv$CANTIDAD)*100)

barplot(joint_counts_id_tpago, beside = TRUE, col=brewer.pal(4,"Paired"),
        ylab = "% Respecto a la cantidad total de artículos vendidos", xlab = "ID_TIENDA")
title("TOMMY: 1079262 artículos @176175 transacciones @427 días 2015-11-01 - 2016-12-31 total = $800,726,791")
legend('topright', levels(TIPO_PAGO), pch=15, col=brewer.pal(4,"Paired"), ncol = 4,  cex = 1)
