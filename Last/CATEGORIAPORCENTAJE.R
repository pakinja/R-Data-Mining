...
CODIGO_SKU <- factor(UBICACION, levels=skus)
...
joint_counts_cat_talla <- (joint_cat_talla$t/sum(pv$CANTIDAD)*100)

barplot(joint_counts_cat_talla, beside = TRUE, col=brewer.pal(10,"Paired"),
        ylab = "% Respecto a la cantidad total de artículos vendidos", xlab = "TALLA")
title("COACH")
...
joint_counts_color <- (joint_cat_color$t/sum(pv$CANTIDAD)*100)

barplot(joint_counts_color, beside = TRUE, col=brewer.pal(10,"Paired"),
        ylab = "% Respecto a la cantidad total de artículos vendidos", xlab = "COLOR")
title("COACH")
legend('topright', levels(CATEGORIA), pch=15, col=brewer.pal(10,"Paired"), ncol = 4,  cex=0.75)

joint_cat_id <- CrossTable(CATEGORIA, ID_TIENDA, prop.chisq = FALSE)
joint_counts_id <- (joint_cat_id$t/sum(pv$CANTIDAD)*100)

barplot(joint_counts_id, beside = TRUE, col=brewer.pal(10,"Paired"),
...
joint_counts_ubicacion <- (joint_cat_ubicacion$t/sum(pv$CANTIDAD)*100)

barplot(joint_counts_ubicacion, beside = TRUE, col=brewer.pal(10,"Paired"),
        ylab = "% Respecto a la cantidad total de artículos vendidos", xlab = "UBICACION")
title("COACH")
legend('topright', levels(CATEGORIA), pch=15, col=brewer.pal(10,"Paired"), ncol = 4,  cex=0.75)
